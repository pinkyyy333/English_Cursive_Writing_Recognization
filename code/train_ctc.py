...
from lstm_utils import decode_label, load_word_datset
...
def evaluate(model, dataloader, criterion, device, idx2char, lm=True, use_beam=True):
    model.eval()
    total_loss = 0
    correct = 0
    total = 0
...
def main():
    ...
    # for IAM line dataset
    # hf_dataset = load_dataset("Teklia/IAM-line")
    
    # for IAM word dataset
    train_dataset_hf, eval_dataset_hf, test_dataset_hf = load_word_datset()
    ...
    lm = kenlm.Model("unique_words.arpa")

    train_dataset = IAMLineDataset(train_dataset_hf, char2idx)
    test_dataset = IAMLineDataset(test_dataset_hf, char2idx)
    
    train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True, collate_fn=collate_fn)
    test_loader = DataLoader(test_dataset, batch_size=4, shuffle=False, collate_fn=collate_fn)

    cnn = CNN()
    pretrained_dict = torch.load("new_cnn_model.pth")
